...
by using the digits found in num. Leading zeros are allowed in new1 and new2, and all the digits found in num must
be used.
...
class Solution:
    def minimumSum(self, num: int) -> int:
        num_string = sorted(list(str(num)))
        return (int(num_string[0]) * 10 + int(num_string[2])) + (int(num_string[1]) * 10 + int(num_string[3]))