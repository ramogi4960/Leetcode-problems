...
class Solution:
    def lastVisitedIntegers(self, nums:[int]) -> [int]:
        seen, ans, index, length, count = [], [], 0, 0, 0
        while index < len(nums):
            if nums[index] > 0:
                seen = [nums[index], ] + seen
                length += 1
                count = 0
            else:
                count += 1
                if count > length: ans.append(-1)
                else: ans.append(seen[count - 1])
            index += 1
        return ans