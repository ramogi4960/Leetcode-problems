...
Input: list1 = ["Shogun","Tapioca Express","Burger King","KFC"], list2 = ["Piatti","The Grill at Torrey Pines","Hungry
...
class Solution:
    def findRestaurant(self, list1: [str], list2: [str]) -> [str]:
        common_string = set(list1).intersection(set(list2))
        final = []
        for item in common_string:
            final.append((item, list1.index(item) + list2.index(item)))

        final.sort(key=lambda x: x[1])
        x = [item[0] for item in final if item[1] == final[0][1]]
        return x