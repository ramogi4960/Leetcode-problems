...
class Solution:
    def numberOfBeams(self, bank: [str]) -> int:
        """
        The logic is, the number of laser beams between any two rows with devices
        is the product of the number of devices of both rows.
        If only one row or none have at least a device, then there's no laser
        """
        devices = []
        for item in bank:
            if n := item.count("1"):  # walrus operator
                devices.append(n)
