...
# define function for heapifying when an item is added
def heapify(arr: list, length: int, index: int) -> None:
    largest = index
    right = index * 2 + 2
    left = index * 2 + 1
    if left < length:
        if arr[left][-1] > arr[largest][-1]:
            largest = left
        elif arr[left][-1] == arr[largest][-1] and arr[left][1] > arr[largest][1]:
            largest = left

    if right < length:
        if arr[right][-1] > arr[largest][-1]:
            largest = right
        elif arr[right][-1] == arr[largest][-1] and arr[right][1] > arr[largest][1]:
            largest = right
...
class TaskManager:
    ...
    def __init__(self, tasks: [[int]]):
        self.tasks_heap = tasks
        self.length = len(tasks)

        for index in range(len(self.tasks_heap) // 2 - 1, -1 -1):
            heapify(self.tasks_heap, self.length, index)

    def add(self, userId: int, taskId: int, priority: int) -> None:
        self.tasks_heap.append([userId, taskId, priority])
        self.length += 1
        heapify(self.tasks_heap, self.length, self.length // 2 - 1)
