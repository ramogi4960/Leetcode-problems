...
class Solution:
    def garbageCollection(self, garb: [str], travel: [int]) -> int:
        garb, travel, count = garb[::-1], travel[::-1], 0
        M = G = P = False
        # 2 + 1 +
        for i in range(len(garb)):
            if i == 0:
                if "G" in garb[i]:
                    count += garb[i].count("G")
                    G = True

                if "P" in garb[i]:
                    count += garb[i].count("P")
                    P = True

                if "M" in garb[i]:
                    count += garb[i].count("M")
                    M = True
